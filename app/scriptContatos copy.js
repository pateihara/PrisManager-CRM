...
function validaAdicionar(nome, status, ultimoContato) {
    ...
    document.querySelector("#formularioAdicionar").reset();
}
...
function exibirTabelaCores() {
    ...
    cores.map(cor => {
        ...
        <td>${cor.color}</td>
        <td><a href="#" title="Editar" data-toggle="modal" data-target="#ModalEditar" onclick="abrirEditorCorStatus(${cor.sku})">Editar</a></td>
        </tr>`;
    })
